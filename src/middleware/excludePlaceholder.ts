...
export const excludePlaceholder = {
  ...
  fromResponse: () => {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      const hasPlaceholder = (item: DataItem, depth: number = 0, seen = new WeakSet()): boolean => {
        ...
        const directValues = Object.values(item).filter(value => typeof value === 'string');
        if (directValues.some(value => value === PLACEHOLDER)) return true;

        // Then check nested objects and arrays
        return Object.values(item).some(value => {
          if (Array.isArray(value)) {
            return value.some(v => typeof v === 'object' && v !== null ? hasPlaceholder(v, depth + 1, seen) : v === PLACEHOLDER);
          }
      ...
      res.json = function(data: any) {
        if (!req.query.includeAll && data) {
          if (Array.isArray(data)) {
            ...
            data.results = data.results.filter((item: DataItem) => !hasPlaceholder(item));
