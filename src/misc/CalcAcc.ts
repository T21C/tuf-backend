...
export function sumJudgements(inp: IJudgements): number {
    return inp.earlyDouble + inp.earlySingle + inp.ePerfect + inp.lPerfect + inp.lateDouble + inp.lateSingle + inp.perfect;
}

export function tilecount(inp: IJudgements): number {
    return inp.earlySingle + inp.ePerfect + inp.lPerfect + inp.lateSingle + inp.perfect;
}

export function calcAcc(inp: IJudgements, raw = false): number {
    // Handle array format (from client)
    
    if (inp) {
        const result = (inp.perfect + // perfect
