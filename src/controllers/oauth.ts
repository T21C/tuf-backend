...
import { findPlayerByDiscordId } from '../utils/playerMapping';
import axios from 'axios';
import { type RESTPostOAuth2AccessTokenResult, type RESTGetAPIUserResult } from 'discord-api-types/v10';
...
export const OAuthController = {
  ...
  async unlinkProvider(req: Request, res: Response) {
    ...
    try {
      ...
      if (providers.length <= 1) {
        return res.status(400).json({ error: 'Cannot remove last authentication provider' });
      }
  ...
  async refreshToken(req: Request, res: Response) {
    ...
    try {
      const providers = await OAuthService.getUserProviders(req.user!.id);
      const oauthProvider = providers.find((p: OAuthProvider) => p.provider === provider);
      ...
      return res.status(400).json({ error: 'Provider not supported for token refresh' });
    } catch (error) {
