...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (isCaptchaRequired(emailOrUsername)) {
      if (!captchaToken) {
        return res.status(400).json({ error: 'Captcha required', requireCaptcha: true });
      }

      const isValidCaptcha = await verifyCaptcha(captchaToken);
      if (!isValidCaptcha) {
        return res.status(400).json({ error: 'Invalid captcha or high risk score detected' });
      }
    }
    ...
    if (!user) {
      ...
      const attempts = failedAttempts.get(emailOrUsername) || { count: 0, timestamp: Date.now() };
      attempts.count++;
    ...
    if (!user.password) {
      return res.status(401).json({ error: 'Please login with your linked provider' });
    }
