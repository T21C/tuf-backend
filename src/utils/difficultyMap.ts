...
import {getIconUrl} from './iconResolver.js';
import {calculateBaseScore} from './ratingUtils.js';
...
export const initializeDifficultyMap = async (
  transaction?: any,
): Promise<DifficultyEntry[]> => {
        ...
        try {
          const newDiff = {
            ...
            legacyIcon: diff.legacyIcon
              ? await cacheIcon(diff.legacyIcon, `legacy_${diff.name}`)
              : null,
          };
          return newDiff;
        } catch (error) {
