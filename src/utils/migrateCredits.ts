...
import {parseCredits} from './creditParser.js';
import {Transaction} from 'sequelize';
import {ILevel} from '../interfaces/models/index.js';

export async function migrateCredits(
  levels: Omit<ILevel, 'submitterDiscordId' | 'createdAt' | 'updatedAt'>[],
  ...
  for (const level of levels) {
    ...
    if (credits.team && level.team && level.team.trim() !== '') {
      ...
      if (
        !level.creator.includes('[') &&
        !level.creator.includes('(') &&
        !level.charter.includes('[') &&
        !level.charter.includes('(') &&
        !level.vfxer.includes('[') &&
        !level.vfxer.includes('(')
      ) {
        await Level.update(
          {isVerified: true},
          {where: {id: level.id}, transaction},
        );
      }
    } else {
      credits.charters.forEach(name => {
  ...
  const existingCreatorsByName = new Map(
    existingCreators.map(c => [c.name, c]),
  );

  // Prepare initial creator records
  const creatorsToCreate = Array.from(uniqueCreatorNames)
  ...
  const creators = await Creator.bulkCreate(finalCreators, {
    validate: true,
  ...
  existingCreators.forEach(creator => {
    logCreatorDebug(
      'Added to creatorMap from existing creators',
      creator.name,
      {
        id: creator.id,
        existingRecord: true,
  ...
  const levelCreditEntries = creditEntries
    .map(entry => {
      ...
      logCreatorDebug(
        'Transforming credit entry to include creator ID',
        entry.creatorName,
        {
  ...
  for (const [lowerName, creatorGroup] of creatorsByName) {
    if (creatorGroup.length > 1) {
      console.log(
        `Found ${creatorGroup.length} creators for name '${lowerName}'`,
      );

      // Sort by level count descending to find the main creator
      creatorGroup.sort(
        (a: CreatorEntry, b: CreatorEntry) => b.levelCount - a.levelCount,
      );
      const mainCreator = creatorGroup[0].creator;
      const duplicates = creatorGroup
        .slice(1)
        .map((c: CreatorEntry) => c.creator);

      console.log(
        `Merging to main creator '${mainCreator.name}' (${creatorGroup[0].levelCount} levels)`,
      );

      // Transfer all credits to the main creator
      for (const duplicate of duplicates) {
        console.log(
          `- Merging '${duplicate.name}' (${duplicate.createdLevels?.length || 0} levels)`,
        );
  ...
  if (creatorsToVerify.length > 0) {
    ...
    console.log(
      `Auto-verified ${creatorsToVerify.length} creators with 20+ levels`,
    );
  }

  console.log(
    `Credit migration completed: ${creators.length} creators, ${teams.length} teams, and ${levelCreditEntries.length} credits created`,
  );
}

export async function migrateNewCredits(
  levels: Omit<ILevel, 'submitterDiscordId' | 'createdAt' | 'updatedAt'>[],
  transaction: Transaction,
): Promise<void> {
  ...
  const existingCreatorsByName = new Map(
    existingCreators.map(c => [c.name.toLowerCase(), c]),
  );
  ...
  const newCreatorNames = Array.from(uniqueCreatorNames).filter(
    name => !existingCreatorsByName.has(name.toLowerCase()),
  );
  ...
  const newTeamNames = Array.from(uniqueTeamNames).filter(
    name => !existingTeamsByName.has(name.toLowerCase()),
  );
  ...
  const levelCreditEntries = creditEntries
    .map(entry => {
      const creator = existingCreatorsByName.get(
        entry.creatorName.toLowerCase(),
      );
      if (!creator) {
  ...
  console.log(
    `Credit migration completed for new levels: ${newCreatorNames.length} new creators, ${newTeamNames.length} new teams, and ${levelCreditEntries.length} new credits created`,
  );
}
