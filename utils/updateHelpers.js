...
export const updateRanks = () => {
  ...
  export const updateData = () => {
    ...
    exec(`python ./parser_module/executable.py all_players --output=${PATHS.playerlistJson} --reverse`, (error, stdout, stderr) => {
      if (error) {
      ...
      exec(`python ./parser_module/executable.py all_clears --output=${PATHS.clearlistJson} --useSaved`, (error, stdout, stderr) => {
        if (error) {
  ...
  export const fetchRatings = async () => {
    const response = await fetch(process.env.RATING_SCRIPT_URL);
    const ratingList = await response.json();
    writeJsonFile(PATHS.ratingListJson, ratingList)
  }
  
  export const updateTimestamp = (name) => {
